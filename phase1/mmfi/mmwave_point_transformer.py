...
class PointTransformerReg(nn.Module):
    def __init__(self, input_dim = 5, nblocks = 5, nneighbor = 16, transformer_dim = 128, n_p = 17):
        ...
        self.joint_template = nn.Parameter(torch.rand(size = (n_p, dim)))
    ...
    def forward(self, x):
        ...
        #################################
        '''
        joint_embedding = self.joint_template.expand(x.shape[0], -1, -1) + self.joint_posembeds_vector
